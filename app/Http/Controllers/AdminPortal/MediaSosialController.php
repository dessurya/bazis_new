...
class MediaSosialController extends Controller
{
	public function index(Request $request){
        ...
        if ($validator->fails()) {
            return redirect()->route('adpor.medsos.index')
				->with('berhasil', 'Terjadi Keselahan Filter');
        }

        $authorList = PublicContent::select('user_id')->where('category', 'media_sosial')->get();

    	$get = PublicContent::orderBy('id','desc')->where('category', 'media_sosial');
        ...
        return view('admin-portal.media-sosial.index', compact(
        	'get',
        	'authorList',
        	'request',
        	'getFilterAuthor'
	...
	public function simpan(request $request){
		...
		DB::transaction(function () use($request) {
			...
			$upload1->save('asset/picture/media-sosial/'.$img_url);
			...
			$save->category = 'media_sosial';

			$save->save();

		    LogUserRecord::medsosAdd($save);
		});
	...
	public function hapus($id){
		...
		DB::transaction(function() use($find){
			File::delete('asset/picture/media-sosial/'.$find->picture);
			$find->delete();
		});
		LogUserRecord::medsosDelete($find);
		return redirect()->route('adpor.medsos.index')
	...
	public function update(request $request, $id){
		...
		if($validator->fails())
		{
		return redirect()->route('adpor.medsos.index', ['id'=>encrypt($id)])
			->withErrors($validator)
			->withInput()
			->with('url', 'update')
			->with('title', $save->title)
			->with('action', route('adpor.medsos.update', ['id'=>encrypt($save->id)]))
			->with('false-form', true);
		}

		DB::transaction(function () use($request, $save) {

		    LogUserRecord::medsosUpdate($save, $request);
			
			if($request->file('picture')){
				File::delete('asset/picture/media-sosial/'.$save->picture);
				$salt = str_random(4);
				$image = $request->file('picture');
				$img_url = str_slug($request->title,'-').'-'.$salt. '.' . $image->getClientOriginalExtension();
				$upload1 = Image::make($image);
