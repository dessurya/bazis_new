...
class ProgramKamiController extends Controller
{
	public function index(Request $request){
        ...
        if ($validator->fails()) {
            return redirect()->route('adpor.prokam.index')
				->with('berhasil', 'Terjadi Keselahan Filter');
        }

        $authorList = PublicContent::select('user_id')->where('category', 'program-kami')->get();

    	$get = PublicContent::orderBy('id','desc')->where('category', 'program-kami');
        ...
        return view('admin-portal.program-kami.index', compact(
        	'get',
        	'authorList',
        	'request',
        	'getFilterAuthor'
	...
	public function simpan(request $request){
		...
		$validator = Validator::make($request->all(), [
			'title' => 'required|max:150|unique:zisju_public_content,title',
		...
		DB::transaction(function () use($request) {
			...
			$upload1->save('asset/picture/program-kami/'.$img_url);
			...
			$save->category = 'program-kami';

			$save->save();

		    LogUserRecord::prokamAdd($save);
		});
	...
	public function hapus($id){
		...
		DB::transaction(function() use($find){
			File::delete('asset/picture/program-kami/'.$find->picture);
			$find->delete();
		});
		LogUserRecord::prokamDelete($find);
		return redirect()->route('adpor.prokam.index')
	...
	public function update(request $request, $id){
		...
		$validator = Validator::make($request->all(), [
			'title' => 'required|max:150|unique:zisju_public_content,title,'.$save->id,
			'content' => 'required|min:20',
		...
		if($validator->fails())
		{
		return redirect()->route('adpor.prokam.index', ['id'=>encrypt($id)])
			->withErrors($validator)
			->withInput()
			->with('url', 'update')
			->with('title', $save->title)
			->with('action', route('adpor.prokam.update', ['id'=>encrypt($save->id)]))
			->with('false-form', true);
		}

		DB::transaction(function () use($request, $save) {

		    LogUserRecord::prokamUpdate($save, $request);
			
			if($request->file('picture')){
				File::delete('asset/picture/program-kami/'.$save->picture);
				$salt = str_random(4);
				$image = $request->file('picture');
				$img_url = str_slug($request->title,'-').'-'.$salt. '.' . $image->getClientOriginalExtension();
				$upload1 = Image::make($image);
